...
proto_A_session_lengths <- data %>%
  filter(Prototype == "proto_A") %>%
  select(sessionLength) %>%
  mutate(sessionLength = as.numeric(sessionLength)) %>%
  as.matrix() %>%
  as.vector()
proto_B_session_lengths <- data %>%
  filter(Prototype == "proto_B") %>%
  select(sessionLength) %>%
  mutate(sessionLength = as.numeric(sessionLength)) %>%
  as.matrix() %>%
  as.vector()
...
proto_A_levels_completed <- data %>%
  filter(Prototype == "proto_A") %>%
  select(LevelsCompleted) %>%
  as.matrix() %>%
  as.vector()
proto_B_levels_completed <- data %>%
  filter(Prototype == "proto_B") %>%
  select(LevelsCompleted) %>%
  as.matrix() %>%
  as.vector()
...
proto_A_daily_sessions_per_user <- data %>%
  filter(Prototype == "proto_A") %>%
  group_by(UserID, as.Date(SessionStart)) %>%
  summarise(sessions = n_distinct(SessionID)) %>%
  as.data.frame() %>%
  select(sessions) %>%
  as.matrix() %>%
  as.vector()
proto_B_daily_sessions_per_user <- data %>%
  filter(Prototype == "proto_B") %>%
  group_by(UserID, as.Date(SessionStart)) %>%
  summarise(sessions = n_distinct(SessionID)) %>%
  as.data.frame() %>%
...
data %>%
  ...
  ggplot(
    aes(
      date,
      cumulative_avg_levels_completed,
      group = Prototype,
      col = Prototype
    )
  ) +
  ...
  geom_errorbar(
    aes(
      ymin = cumulative_levels_completed_ci_lower,
      ymax = cumulative_levels_completed_ci_upper
    ),
    width = 0.15
  ) +
  ggthemes::theme_tufte() +
...
data %>%
  ...
  geom_errorbar(
    aes(
      ymin = cumulative_sessions_ci_lower,
      ymax = cumulative_sessions_ci_upper
    ),
    width = 0.15
  ) +
  ggthemes::theme_tufte() +
